...
result = tuple((x ** 2 for x in range(1,11)))  #소괗호1나개생략가능
print(result)
...
# result = tuple(x for x in hours[1:6])                                                    # ✅ 15시간
# print(sum(result))

print(sum(hours[1:6]))

# 2️⃣ 가장 많이 공부한 시간 출력하기
# result = tuple(x for x in hours)                                                    # ✅ 6시간
# print(max(result))
...
# result = tuple(x for x in scores)
# print(max(scores), min(scores))
result = sorted(scores)
print(result[-1], result[0])                                                    # ✅ max 점수: 92점, min 점수: 76점
...
total = sum(price * qty for name, price, qty in stocks)
total = sum([price * qty for name, price, qty in stocks])
